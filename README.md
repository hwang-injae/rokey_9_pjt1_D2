...
- 실패하면(예외 처리 · **9/29 오전 실기로 확인한 것만 시연 대본에** · E45): 빈 구역 → 건너뛰고 HOME(#98) / **잔반이 계속 남음 → 사람 없이 위로 → HOME → 격리 → HOME → 다음 용기**(E52 · tune2 · 🟡 9/29 ④ · 동결판 main 은 멈춤 E42) / 안착 실패 → 격리 / **툴 집기 실패 → 멈춤 → 홀더 확인 → 톡톡 → 다시 집기**(E52) / **닦는 중 툴 놓침 → 그 자리 정지 → 홀더에 넣고 톡톡(15 N · 1번) 또는 화면 재개 → 다시 집고 이어감**(E37 · E48) / **무게 떨림 > 80 g(케이블 당김) → 멈춤 → 케이블 정리 → 톡톡 → 재검증 → 안전정지 자동 해제 → 이어감**(E43 · E47) / 용기 놓침 → 멈추고 사람이 확인 / **로봇 오류 → 그 자리 멈춤(격리로 안 감) → 쥔 것이 있으면 톡톡·재개 → 손만 열기 → 사람이 받음 → 화면 재개 버튼 → 위로 → HOME → 다음 용기**(E52 · E54). 놓기·적재·툴 반납은 힘 감시 없이 곧게 내려가 놓는다(E44 · 걸림은 충돌 감지). 화면 버튼: 정지(즉시) · 재개 · 중단(위로 → HOME → 툴 반납 → 홈 용기 재파지 → 격리 → HOME · E11 · E52 정리 통일).
...
## 3. 지금까지 된 것 (9/23 동결 · `v1.0-demo` = main 8874dfc)

검증 수준 표기: **자동** = pytest(로봇 없이) · **가상** = RViz 가상 로봇 · **실기** = 실제 로봇 · 🟡 = 아직 실기로 못 봄

| 영역 | 상태 | 확인 수준 | 근거 |
|---|---|---|---|
| **시연 경로 리허설** | 메인 프로그램(`flow_node`) 한 번 시작 → **그릇 2 → 컵 2 연속 완주 3회**(배속 0.5 · 19분 34초 / 19분 39초 / 튜닝 뒤 **17분 39초** · 격리 0 · 잔반 루프 1회 · ros bag) — 2번째 슬롯 집기 · 팔레트 B1·B2·C1·C2 · 잔반 감지 → 털기 → 재측정 통과 | 실기 | `docs/test_logs/20260923_VER-0923_*` §17~18 · PR #96 #97 |
...
| 새 기능(9/23) | 그릇 격리 경로(#90 · 중단 정리에서 씀 · #103 흐름 갈래는 E52 로 다시 사용 · tune2) · 세제 펌프(#100 · 제품 함수 · **기본 꺼짐** · 9/29 구매 뒤 결정) · 케이블 이상 → 멈춤 → 톡톡 재개(#93) + 안전정지 자동 해제(#102 · E43) · 툴 놓침 TOOL_LOST → 정지 → 넛지 → 재PICK(#99 · E37) · 넛지 힘 15 N 통일 · 1번(E48 · 9/24 22:1x 2번 철회) | 실기(케이블 1회 · 툴 놓침 rig) · 🟡 **흐름 안 예외 7종 + 빈 시작은 9/29 오전 INT-4b**(E52 재설계 · tune2 21e10e7 자동 시험만) | PR #90 #93 #99 #100 #102 #103 · 대본 `docs/test_logs/20260929_INT-4b_*`(PR 대기) |
...
**동결 뒤(9/24~)**: 추석(9/24~28)은 로봇 없이 화면(HMI)·문서만. **9/29(화) 오전** 브링업 → 좌표 재현 10분 → 리허설 1회(0.5) → 대용품 94 g 무게 확인 → 예외 시나리오 5종 실기(INT-4b 대본) → 기준값 2종 → **14:00 최종 시연**(정상 흐름 + 오전 통과한 시나리오만 · E28 · E45). 동결 뒤 코드는 시연 경로 무영향 안전장치만, 실기 확인 뒤 반영.
자동 시험은 **532개**(9/23 동결 · 두 환경에서 통과). 시험 기록은 `docs/test_logs/`, 결정은 `docs/meetings/20260919_결정기록_DSN-03.md`(E1~E52). 9/23 merge 된 PR: #89~#105(튜닝 #105 까지 = `v1.0-demo`).
...
| 결정 기록 (E1~E52 · 왜 그렇게 했나) | [docs/meetings/20260919_결정기록_DSN-03.md](docs/meetings/20260919_결정기록_DSN-03.md) |
