...
- 실패하면(예외 처리 · **9/29 오전 실기로 확인한 것만 시연 대본에** · E45): 빈 구역 → 건너뛰고 HOME(#98) / **잔반이 계속 남음 → 멈춤 → 사람이 재개(다시 잼) 또는 중단(HOME → 격리 → 다음 용기)**(E42) / 안착 실패 → 격리 / **닦는 중 툴 놓침 → 그 자리 정지 → 홀더에 넣고 톡톡(15 N · 2번) 또는 화면 재개 → 다시 집고 이어감**(E37 · E48) / **무게 떨림 > 80 g(케이블 당김) → 멈춤 → 케이블 정리 → 톡톡 → 재검증 → 안전정지 자동 해제 → 이어감**(E43 · E47) / 용기 놓침·로봇 오류 → 멈추고 사람이 확인. 놓기·적재·툴 반납은 힘 감시 없이 곧게 내려가 놓는다(E44 · 걸림은 충돌 감지). 화면 버튼: 정지(즉시) · 재개 · 중단(HOME → 툴 반납 → 격리 → HOME · E11).
...
| 새 기능(9/23) | 그릇 격리 경로(#90 · 중단 정리에서 씀 · #103 흐름 갈래는 E42 로 미사용) · 세제 펌프(#100 · 제품 함수 · **기본 꺼짐** · 9/29 구매 뒤 결정) · 케이블 이상 → 멈춤 → 톡톡 재개(#93) + 안전정지 자동 해제(#102 · E43) · 툴 놓침 TOOL_LOST → 정지 → 넛지 → 재PICK(#99 · E37) · 넛지 힘·횟수 통일 15 N · 2번(E48 · 코드 진행) | 실기(케이블 1회 · 툴 놓침 rig) · 🟡 **흐름 안 예외 5종은 9/29 오전 INT-4b** | PR #90 #93 #99 #100 #102 #103 · 대본 `docs/test_logs/20260929_INT-4b_*`(PR 대기) |
...
자동 시험은 **532개**(9/23 동결 · 두 환경에서 통과). 시험 기록은 `docs/test_logs/`, 결정은 `docs/meetings/20260919_결정기록_DSN-03.md`(E1~E48). 9/23 merge 된 PR: #89~#105(튜닝 #105 까지 = `v1.0-demo`).
