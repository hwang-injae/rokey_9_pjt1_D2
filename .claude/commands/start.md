...
진행 규칙:
- 각 단계마다 ① 왜 하는지 한 줄 ② 실행할 명령 하나(복붙 가능) ③ 기대 결과. 네가 직접 실행할 수 있는 확인 명령(`git --version`, `git config --global --list`, `echo $REWASH_WS`, `ls ~/ws_cobot_pjt/ws_dsr/install 2>/dev/null`, `ros2 --version`, `which colcon`)은 네가 실행해서 판정한다. 로봇을 움직이는 명령과 `sudo`·설치 명령은 사용자에게 실행을 부탁하고 출력을 붙여 달라고 한다.
...
3. 저장소 위치 확인: 현재 폴더가 clone된 `rokey_pjt01_ws`인지(`git remote -v`), `.bashrc`의 `REWASH_WS`
