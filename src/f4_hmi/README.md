...
| 프로그램 | 역할 | 실행 |
|---|---|---|
| `hmi_bridge` | ROS 방송(`/flow/state` 등)을 듣고 브라우저에 전달하는 서버 | `ros2 run f4_hmi hmi_bridge` → http://localhost:8000 |
| `fake_state_pub` | **가짜 flow** — 대본대로 실제 flow 와 같은 토픽을 방송(로봇·브링업 불필요) | `ros2 run f4_hmi fake_state_pub [대본] [--speed N] [--once]` |
...
- [ ] F4-02 버튼(start·stop·resume·abort) · WebSocket `/ws/state`
