# -*- coding: utf-8 -*-
"""flow — 공정의 **두뇌** (민범진). 순서 · 계획 · 실패했을 때의 처리.

═══════════════════════════════════════════════════════════════════════════════════════
🔰 처음 보는 사람에게 — 이 파일은 "무엇을 어떤 **순서**로, 실패하면 **어떻게**" 를 정한다.
═══════════════════════════════════════════════════════════════════════════════════════

  용기 한 개가 지나가는 순서 (process_one 함수 안의 steps 목록이 그대로 이 순서다)

    PICK   반납 구역에서 집는다                     f1.pick
    WEIGH  저울 자리로 가서 잔반을 잰다 · 많으면 턴다  f1.move_to → f2.leftover_loop
    SEAT   스펀지 고정틀 홈에 내려놓는다             f1.place
    SOAP   수세미를 집는다 → 세제에 담근다           f1.tool → f3.soap
    WIPE   안쪽을 닦는다 → 수세미를 돌려놓는다        f3.wipe_* → f1.tool
    RINSE  홈에서 다시 집는다 → 헹굼물에 담근다 → 물을 턴다   f1.pick → f2.dip → f2.shake
    RACK   팔레트 칸에 꽂는다 → HOME 으로 돌아간다    f1.rack_place → f1.move_to

  ── 순서나 횟수를 바꾸고 싶다면 ────────────────────────────────────────────────────
    횟수·계획·실패 처리는 전부 **params.yaml 의 flow 절**에 있다(이 파일이 아니다).
    단계의 **순서 자체**를 바꾸는 것은 팀 약속(IRD §8)이라 혼자 바꾸지 않는다.

  ── 실패하면 어떻게 하나 (params.yaml 의 flow.policy) ─────────────────────────────
    pause        멈추고 **사람을 기다린다**. 사람이 '다시' 를 누르면 그 단계부터 다시 한다
    isolate      이 용기를 격리 자리에 치우고 다음 용기로 간다
    retry:1->isolate   한 번 더 해 보고, 그래도 안 되면 격리한다
    next_zone    이 구역에 용기가 없다는 뜻 → 다음 구역으로 간다

  ── 이 파일에서 자주 나오는 말 ────────────────────────────────────────────────────
    깃발        사람이 누른 버튼(시작·정지·다시·중단)을 적어 두는 표시.
                버튼을 누르면 깃발이 서고, 이 파일이 단계 사이마다 깃발을 들여다본다.
                🔔 버튼을 누른 그 자리에서 로봇을 건드리지 않는 이유는 flow_node.py 머리말에 있다.
    상태(step)  지금 어느 단계인지를 가리키는 글자. PICK · WEIGH · … · PAUSED · DONE.
                화면(HMI)에 그대로 보인다.
    기록(event) 용기 한 개가 끝날 때 남기는 한 줄. 무게·걸린 시간·결과가 들어간다.
