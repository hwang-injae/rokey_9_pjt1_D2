# -*- coding: utf-8 -*-
"""움직이기 전에 확인하는 것들 — 민범진 · 9/22

═══════════════════════════════════════════════════════════════════════════════════════
🔰 처음 보는 사람에게 — 로봇이 첫 걸음을 떼기 **전에** 세 가지를 확인한다.
═══════════════════════════════════════════════════════════════════════════════════════

  ① 손끝 설정이 맞나 (require_controller)
     로봇은 "손끝이 어디인가" 를 설정값으로 들고 있다(이름: GripperDA_v1). 그리퍼 길이만큼
     앞으로 나간 지점이 손끝이다. 이 설정이 풀리면 로봇은 **그리퍼가 없다고 생각**해서
     같은 좌표 명령을 208 mm 아래로 보낸다 → 그릇이 바닥에 부딪힌다(9/22 실제 사고 · TS-07).
     여러 사람이 한 로봇을 쓰니 남이 풀어 둘 수 있다 → **다르면 시작을 거부한다.**

  ② 케이블이 팽팽하지 않나 (warn_if_cable_tight)
     그리퍼 케이블이 당겨져 있으면 팔이 가만히 서 있어도 무게 값이 크게 흔들린다.
     HOME 에서 5 초쯤 힘을 읽어 보고 흔들리면 **경고만** 한다(멈추지는 않는다).

  ③ 지금 자세가 너무 낮지 않나 (go_home_safely)
     헹굼·담금은 **수조 안 자세**에서 끝난다. 거기서 HOME 으로 곧장 가면 팔이 테이블 높이를
     가로질러 **그리퍼가 상판을 쓴다**(9/22 실제 사고 · TS-08).
     그래서 HOME 으로 가기 전에 **곧게 위로 먼저 빠져나온다.**

  🔔 ①은 확인만 하고 로봇을 움직이지 않는다. ③은 안전을 위해 위로만 움직인다.
