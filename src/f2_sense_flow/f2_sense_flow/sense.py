...
═══════════════════════════════════════════════════════════════════════════════════════
🔰 처음 보는 사람에게 — 이 파일은 "어떻게 움직이나" 를 적은 곳이다. **숫자는 여기 없다.**
═══════════════════════════════════════════════════════════════════════════════════════

  이 파일이 맡은 일은 네 가지다. 사람이 설거지할 때 하는 일과 그대로 짝이 된다.

    weigh          그릇을 들고 저울 자리로 가서 **잔반이 얼마나 남았나** 잰다
    leftover_loop  재고 → 많으면 털고 → 다시 재고 → 그래도 많으면 포기(격리). weigh 를 안에서 부른다
    shake          잔반통이나 수조 **위에서** 손목을 왕복시켜 **턴다**
    dip            수조 위에서 아래로 내려갔다 올라온다 (🚨 실제 물은 안 쓴다 — 동작만 한다)

  ── 숫자(각도·깊이·횟수)를 바꾸고 싶다면 ─────────────────────────────────────────────
    이 파일을 고치지 않는다. 전부 **src/cobot_common/config/params.yaml 의 f2 절**에 있다.
    그 파일 맨 위에 "무엇을 바꾸고 싶은가 → 어느 값을 고치나" 표를 적어 두었다.
    고친 뒤에는 터미널에 **cbc** 를 쳐야 로봇에 전달된다.

  ── 로봇이 서는 자리(좌표)를 바꾸고 싶다면 ──────────────────────────────────────────
    좌표는 **src/cobot_common/config/cell.yaml** 에 있다(stations.WEIGH · WASTE · RINSE …).
    자리를 눈으로 찾을 때는 test/rig_jog.py 로 관절을 조금씩 움직이고 p 키를 누르면 지금 좌표가 찍힌다.

  ── 이 파일의 함수를 하나씩 돌려 보고 싶다면 ────────────────────────────────────────
    test/rig_f2.py 가 그 도구다. 사용법은 그 파일 맨 위에 적혀 있다.
      예) rig_f2.py dip --kind BOWL --count 2      ← 헹굼 담금만 돌려 본다

  ── 이 파일에서 자주 나오는 말 (한 번만 읽어 두면 된다) ─────────────────────────────
    티칭 자세   사람이 로봇 팔을 직접 끌어다 "여기서 시작해" 하고 기억시킨 자세.
                cell.yaml 에 좌표로 적혀 있다. 각 동작은 이 자세에서 시작한다.
    접근점      티칭 자세 **바로 위**의 자세. 수조·저울로 갈 때 비스듬히 가면 부딪히니까,
                먼저 위쪽(접근점)까지 간 다음 **곧게 아래로** 내려간다.
    쥐는 힘     NORMAL(보통) / HOLD(더 꽉). 흔들거나 담글 때만 HOLD 로 바꿨다가 끝나면 되돌린다.
    영점        빈손으로 그리퍼를 끝까지 닫아도 폭이 0 이 아니라 10.58 mm 로 읽힌다. 그 값이 영점이다.
                "쥐었나" 를 볼 때는 읽은 폭에서 이 영점을 빼고 본다.
    힘제어      로봇이 밀리는 힘을 느끼며 부드럽게 움직이는 방식. 닦기처럼 물체에 닿을 때만 켠다.
                켜 둔 채 다른 곳으로 이동하면 명령대로 안 가므로, 이동 전에 반드시 끈다(cc.force_off).
    표본/중앙값 무게를 여러 번 읽은 값들이 표본이고, 그 가운데 값이 중앙값이다.
                한 번 크게 튄 값이 있어도 가운데 값은 끌려가지 않아서 평균보다 믿을 만하다.
    vel_scale   전체 속도 배율. 터미널에서 PREWASH_VEL_SCALE=0.3 을 앞에 붙이면 30 % 속도로 움직인다.
                🚨 처음 돌리는 동작은 항상 0.3 으로 본다.
    시험대(rig) 로봇 동작을 사람이 직접 하나씩 돌려 보는 프로그램. test/ 폴더의 rig_*.py 가 그것이다.
...
def _via_home():
    ...
    잔반통 그릇 자세를 뒤쪽으로 옮기면서 생긴 제약이다. 앞쪽 자세는 팔이 거의 쭉 펴진 자세인데,
    이렇게 펴진 자세에서는 손끝을 조금만 움직여도 관절이 크게 돌아간다 —
    9/21 08:40 실기에서 6번 관절이 163° 나 돌아 그리퍼 케이블이 꼬였다.
...
def weigh(kind: str) -> WeighResult:
    ...
    🚨 왜 '측정값' 이 아니라 '잔반 무게' 인가: 이 함수가 kind 를 받는 이유가
       **빈 용기 기준값을 고르기 위해서**다(SDD §5.3). 로봇이 읽는 무게에는 원인 모를 **치우침**이
       얹혀 있는데(저울의 0 이 어긋난 것과 같다 · V-02: +42~45 g), 같은 길로 같은 자세에서 잰
       빈 용기 값을 빼면 그 치우침이 **서로 지워진다**.
...
def leftover_loop(kind: str, max_rounds: int) -> LeftoverResult:
    ...
    🔔 알려진 한계: 이 함수 한 덩어리가 flow 기준 **한 단계**라, 도는 동안 정지 버튼을 못 본다
       (flow 는 단계 사이마다 본다 — SDD §5.1). 중단 신호를 중간에 받으려면 함수 모양이 바뀌므로
