"""F2 함수를 **하나씩 따로 돌려 보는 도구** — 민범진 (SDD §3.2 · §10)

═══════════════════════════════════════════════════════════════════════════════════════
🔰 이게 무슨 프로그램인가
═══════════════════════════════════════════════════════════════════════════════════════
  전체 공정을 돌리지 않고 **내가 만든 동작 하나만** 떼어서 돌려 본다.
  값을 고친 뒤 "이 동작만 확인하고 싶다" 할 때 쓴다.

    release   그리퍼를 연다 (용기를 손으로 쥐여 주기 전에 먼저 한다)
    grip      cell.yaml 에 적힌 폭·힘으로 **쥔다**
    empty     🔑 **빈 용기 기준값을 잰다** — 결과 숫자를 params.yaml 의 f2.empty_weight_g 에 적는다
    weigh     잔반 무게를 잰다 (기준값을 빼고 나온 값)
    loop      재고 → 무거우면 털고 → 다시 재고 (실제 공정과 같은 순서)
    shake     흔들어 턴다 (--mode WASTE 잔반 털기 / --mode RINSE 물 털기)
    dip       수조에 담갔다 올라온다

───────────────────────────────────────────────────────────────────────────────────────
🔰 하는 순서 — 용기를 쥐는 것부터
───────────────────────────────────────────────────────────────────────────────────────
  무게·털기·담금은 전부 **용기를 쥔 상태**에서 시작한다. 그래서 먼저 쥐여 줘야 한다.

    ① 터미널 1 에서 로봇을 켠다      sod && sodreal
    ② 터미널 2 에서 빌드            cbc
    ③ 그리퍼를 연다
         python3 src/f2_sense_flow/test/rig_f2.py release
       → 손으로 그릇 옆벽을 그리퍼 사이에 **세로로** 대 준다 (컵은 몸통을 통째로)
    ④ 쥔다
         python3 src/f2_sense_flow/test/rig_f2.py grip --kind BOWL
       → 화면의 '영점 뺀 폭' 이 2 mm 근처면 잘 쥔 것이다. 10 mm 근처면 빈손이다
       🔔 그리퍼는 프로그램이 끝나도 **쥔 채로 남는다.** 그래서 ③④ 와 ⑤ 를 다른 명령으로 나눠 해도 된다
    ⑤ 원하는 동작을 돌린다 (처음에는 꼭 30 % 속도로)
         PREWASH_VEL_SCALE=0.3 python3 src/f2_sense_flow/test/rig_f2.py dip --kind BOWL --count 2

───────────────────────────────────────────────────────────────────────────────────────
🔰 자주 쓰는 명령
───────────────────────────────────────────────────────────────────────────────────────
    rig_f2.py empty --kind BOWL         빈 용기 기준값 측정 → 나온 숫자를 params.yaml 에 적는다
    rig_f2.py weigh                     무게 측정 3회
    rig_f2.py loop --max-rounds 2       재고 → 털고 → 다시 재기
    rig_f2.py shake --mode WASTE -n 5   잔반 털기 5회
    rig_f2.py dip --kind CUP            컵 담금
    rig_f2.py weigh --no-robot          🔔 로봇 없이 — 함수가 무엇을 돌려주는지만 확인(움직이지 않는다)

  같은 동작을 **연속 3번** 돌리는 것이 기본이다(-n 으로 바꾼다). "첫 번째만 되는" 문제는
  한 번만 돌려서는 보이지 않기 때문이다(SDD §3.2 ⑧).

  값을 바꿔 가며 눈으로 맞추고 싶다면 이 프로그램보다 rig_shake_tune.py 가 낫다(키보드로 바로 바꾼다).
  이 프로그램에서도 --amp / --period / --tilt 로 이번 한 번만 값을 덮어쓸 수 있다.

───────────────────────────────────────────────────────────────────────────────────────
🔰 안전
───────────────────────────────────────────────────────────────────────────────────────
  🚨 로봇을 움직이기 전에 `rosinfo` 로 RANGE=LOCALHOST 인지 확인한다 (AGENTS §3 규칙 13).
     내 PC 가 다른 PC 와 이어져 있으면 **내 명령이 남의 로봇에도 간다.**
  · 시작할 때 컨트롤러의 툴·TCP 이름을 확인한다(다르면 시작 거부 · TS-07)
  · 낮은 자세(수조 안 등)면 곧게 위로 올라온 뒤 HOME 으로 간다(TS-08)
  · 무게를 잴 때는 용기 안에 넣을 무게추가 필요하다(작은 것 43 g · 큰 것 64 g)
