# -*- coding: utf-8 -*-
"""무게 재기 — 담당 민범진 (INF-02c). 함수 표는 docs/03_설계_SDD.md §3.1.

═══════════════════════════════════════════════════════════════════════════════════════
🔰 처음 보는 사람에게 — 우리 셀에는 **저울이 없다.** 로봇 팔이 저울 노릇을 한다.
═══════════════════════════════════════════════════════════════════════════════════════

  로봇은 팔을 버티느라 관절에 드는 힘을 늘 알고 있다. 물건을 들면 그 힘이 조금 늘어난다.
  그 차이를 무게로 바꿔 주는 것이 이 파일이다. 손에 물건을 얹으면 팔이 묵직해지는 것과 같다.

  그래서 **정밀한 저울이 아니다.** 알고 써야 할 성질이 세 가지 있다.

    ① 값이 들쭉날쭉하다     한 번만 읽으면 못 쓴다 → 여러 번 읽고 **가운데 값**을 쓴다.
                            (가운데 값은 한 번 크게 튄 값에 끌려가지 않는다. 평균보다 낫다)
    ② **0 이 어긋나 있다**   빈손인데도 −89 g 처럼 읽힌다. 이 어긋남을 '치우침' 이라고 부른다.
                            그래서 **빼기**로 쓴다 — (지금 값) − (빈 용기일 때 값) = 잔반 무게.
                            두 값을 같은 자세에서 재면 치우침이 서로 지워진다.
    ③ **0 이 시간이 지나면 움직인다**  몇 시간 전에 재 둔 '빈 용기일 때 값' 은 못 쓴다.
                            (9/22: 5시간 만에 105 g 이동) → 기준값은 **그날, 가까운 시각에** 다시 잰다.
                            재는 21 초 동안에도 값이 한 방향으로 흐르면 이 파일이 경고를 띄운다(📉).

  ── 기준값을 다시 재는 법 ──────────────────────────────────────────────────────────
      빈 그릇을 쥐여 준 상태에서
        python3 src/f2_sense_flow/test/rig_f2.py empty --kind BOWL
      나온 숫자를 params.yaml 의 f2.empty_weight_g.BOWL 에 적는다.
      🚨 **저울로 잰 진짜 무게(47 g)를 적으면 안 된다.** 로봇이 읽은 값을 적어야 한다.

아래는 왜 그냥 받아 쓰면 안 되는지에 대한 자세한 기록이다:
...
🚨 **치우침(옵셋)** 에 대해 — 읽은 값에는 늘 일정한 치우침이 얹혀 있다(저울의 0 이 어긋나 있는 것과 같다).
   빈손인데 −89 g(9/22 · 툴 무게 등록 뒤) 처럼 **음수**로 읽히기도 한다.
...
def _try_reset(d, conf):
    ...
    판정을 **빼기**(측정값 − 빈 용기 기준값)로 하기 때문에, 0점을 못 맞춰도 치우침은 서로 지워진다.
    """
