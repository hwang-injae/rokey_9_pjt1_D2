...
- **그 밖의 PR**(`src/`·`config`·`.github/`·`tools/`·루트 문서·인터페이스 정본): 자동 merge 하지 않는다(9/19 변경 — 로봇을 움직이는 코드가 아무도 읽지 않은 채 main에 들어가지 않게). PM 에이전트가 검토해 PM(황인재)에게 보고하고, 황인재가 확인하면 승인·Squash merge 한다. 보통 몇 분~몇십 분. 급하면 팀 채널에 PR 번호를 알린다.

### 리뷰·merge
| 항목 | 규칙 |
|---|---|
| 필요 승인 | **황인재 1명** (GitHub 규칙: Code Owner 승인 필수). **문서만 바뀐 PR**은 Actions가 §4.1 검사를 통과하면 PM 토큰으로 승인하고 Squash merge까지 한다(2~3분). **코드·설정·도구·인터페이스 정본이 포함된 PR**은 PM 에이전트 검토 → 황인재 확인 뒤 승인·merge. 보류하려면 제목에 `[hold]` 또는 라벨 `hold` |
| 응답 시간 | 4시간 안에 코멘트 또는 승인 |
| Merge 방식 | Squash and merge (문서 PR은 자동, 코드 PR은 검토 뒤 황인재). 실패하면 황인재가 수동 |
