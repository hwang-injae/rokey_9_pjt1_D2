...
| 상태 | **종결 — 구조를 바꿔 문제의 자리를 없앴다.** 9/18 저녁 Virtual(실제 `dsr_controller2`)에서 구조 4종을 비교(§7)하고 **스크립트형(구조 ④)** 을 채택: [DSN-02b](../meetings/20260918_DSN-02b_구조변경_스크립트형.md) |
| 반영 | IRD v3.0(서비스 12개 → 함수 12개) · SDD §1·§3.1(`init` `force_reached`)·§3.2(실행 뼈대 규약)·§5.1 · AGENTS.md §4·§5 · `cobot_api` 신설 · `cobot_msgs` v3.0 · 환경설정 §12 |
...
## 4. 해결 (1차안 — 노드·서비스 구조를 유지할 때. **§7에서 구조 ④로 대체됨**)
...
## 7. 구조 비교 시험과 최종 해결 (9/18 저녁, Virtual + 실제 `dsr_controller2`)
§4의 1차안을 실제 드라이버로 확인하면서 구조 4종을 같이 돌렸다. 스크립트: [ts01_repro/virtual/](ts01_repro/virtual/) (`t1.sh` 번갈아·연속 호출, `t2.sh` 구조 ③·④, `t3.sh` 겹침).

| 구조 | 결과 |
|---|---|
| ① 기능 노드 3개(프로세스 3개) + 각자 DSR 전용 노드 (§4) | 번갈아 9회 성공, 같은 노드 연속 3회 성공, 관절 값 변화 확인 → **§4는 실제 드라이버에서도 동작한다** |
| ① 에서 명령이 겹칠 때 | 노드 A가 +40°로 가는 중 0.5 s 뒤 노드 B가 −40° 명령 → 로봇은 −40°로 갔고 **A도 B도 "성공"을 돌려줬다**(2회 중 2회). 오류·거절 없음. 프로세스가 달라 서로 막을 수 없다 |
| ③ 한 프로세스에 기능 노드 3개 + DSR 노드 공유 + 잠금 | 번갈아 9회 성공, 겹친 요청은 `BUSY` 거절 |
| **④ 스크립트형** — 기능은 함수, 메인 스레드가 차례로 실행, 통신 노드(start/stop 서비스·상태 타이머)만 백그라운드 실행기 | **연속 6회 성공, 모션 중 상태 2.000 Hz, 모션 중 stop 수락 → 다음 함수 전에 보류** |
| 두산의 클래스 API `CDsrRobot` ([cls.py](ts01_repro/cls.py)) | 대기 방식이 같아 서비스 콜백 안에서 같은 오류. 게다가 우리 설치본은 서비스 이름을 `/motion/move_joint`로 찾아(실제는 `/dsr01/dsr_controller2/motion/move_joint`) **스크립트에서도 무한 대기** → 쓰지 않는다 |

**최종 해결 = 구조 ④.** 서비스 콜백 안에서 로봇을 움직이지 않으므로 B·B′가 생길 자리가 없고, 로봇 명령을 내는 곳이 메인 스레드 하나라 겹침도 없다. A(초기화 순서)와 D(반환값)는 `cobot_common`이 계속 막는다. 규약은 SDD §3.2.

남은 확인: ① 팀 코드로 실행 뼈대 재확인(V-20) ② 동작 중 소프트 정지 — 비동기 이동 + 폴링(V-24, 미검증) ③ 힘·무게·접촉은 Virtual에 없으므로 실기 검증(V-01~19)은 그대로.
