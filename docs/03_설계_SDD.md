...
| 재개 · 중단 | PAUSED 에서만. **재개** = 멈춘 단계부터 이어서 · **중단** = 그 용기를 접고(HOME → 툴 반납 → 격리 → HOME · E11) 다음 용기. 로봇 오류(ROBOT_ERROR) 멈춤에서는 중단 비활성 — 단 **케이블 이상**(코드는 ROBOT_ERROR + '케이블' 문구)은 중단 가능(`flow.handle_cable_tight` 와 일치). 누르면 flow 의 대답 문구를 그대로 보여 준다(응답 ≤ 1 s · TC-11) |
