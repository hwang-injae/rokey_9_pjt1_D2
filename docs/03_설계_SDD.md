...
| `TOOL_LOST` 🆕 E37(9/23) | 닦는 중(soap · wipe) 그리퍼 폭이 기준에서 `slip_tol_mm` 넘게 벗어남(툴 놓침 · f3 감시 스레드가 `halt`) | **그 자리 정지 → PAUSED + 알림** → 사람이 툴을 홀더에 넣고 **넛지(15 N · 2번 · E48) 또는 HMI 재개** → 로봇 STANDBY 확인 → `f1.tool(PICK)` 재호출(집은 자리로 곧장) → 놓친 단계부터 이어감(PR #99). 격리 아님 | 경고 |
