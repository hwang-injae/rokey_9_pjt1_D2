...
sequenceDiagram
  ...
  Note over W: 🔄 E25(9/22) — kind 가 params `flow.weigh_kinds`(기본 [BOWL])에 없으면 WEIGH 두 단계(move_to WEIGH · leftover_loop)를 건너뛴다(컵은 액체만 · 잔반 없음 · PR #70 FLOW-05). RINSE 담금·물 털기는 컵도 한다
...
- `shake`: 🔄 **E24(9/22) — `mode` 마다 다른 동작이다**: `WASTE` = **잔반 버리기** — ✅ PR #68: 흔들기 **전에** J5 를 `f2.shake.WASTE.tilt_deg`(−90 · V-07 실기)만큼 기울여 입을 잔반통 쪽으로 → 그 자세를 가운데로 ±amp 흔들고 → 되돌린다(실패하면 finally 가 되돌림 · 상한 `f2.limits.max_tilt_deg` 100) · 잔반통 자세 J6 180(그릇이 잔반통 위로 · 케이블 주의) · 🟡 컵은 V-07 미확인 / `RINSE` = **물기 털기** — ✅ PR #70: **BASE X 축 직선 왕복**(`f2.shake.RINSE: {axis: x, amp_mm: 20, period_s: 0.5}` · `cc.move_rel` 로 가운데 → +amp → −amp → 가운데 · 상한 `f2.limits.max_amp_mm` 60 · 직선 왕복에는 `tilt_deg` 불가 · 실패하면 finally 가 되돌림 · 🟡 amp 20 은 시작값, V-07 에서 수조 폭 재고 확정). 서명은 그대로. — (옛 방식) 관절 왕복: 티칭 자세(수조 **위**)에서 J5/J6 관절 왕복 — `cc.move_joint_rel(joint, ±amp, time_s=…)`. 🚨 `time_s`는 **한 번 움직이는 구간의 시간**이다(가운데 → 끝 = `period_s/4`, 끝 → 반대쪽 끝 = `period_s/2`) — `period_s`를 그대로 넘기면 4배 느려진다. 평균 속도가 `cell.motion.vel_joint_max_deg_s × vel_scale`을 넘으면 자동으로 느려진다(#16). 충돌 감지 오작동 시 진폭 축소(V-07).
