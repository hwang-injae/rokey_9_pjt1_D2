...
### 📦 산출물 (상대경로)
`src/f4_hmi/app.py` `db.py` `static/index.html` `fake_state_pub.py` · `config/params.yaml`의 `hmi` 절 · `kpi.py` · `src/cobot_msgs/` · `src/prewash_bringup/launch/*.py` · `config/` 골격 · 아키텍처 `.drawio` · 회의록 · 시연 영상·PPT
...
9/18 C F4-00 HMI 설계 초안·F4-01 fake pub·골격·INF-01b msgs v2.1·INF-04 config 골격·런치 → 9/19 A ENV-03 통신, A~B F4-02 브리지·버튼·ARCH-01, C~9/20 B F4-03 화면 → 9/20 A V-13, B F4-04 SQLite, C **UT-F4** → 9/21 C **INT-4** → 9/19 C DSN-03 회의·DSN-04 반영 → 9/22 A NOTE-01·NOTE-02·F4-05 KPI, B~C L3 HMI(PC-B) → 9/23 INT-4c 측정·영상 촬영 → 추석 영상 편집·PPT
