...
### 담당 작업
- **9/18 저녁 최우선**: `mock_f1_f3`(같은 서비스 이름으로 즉시 응답 + 실패 주입) — flow 개발의 전제. `cobot_msgs`는 PM(황인재)이 `docs/interfaces/`에서 복사해 관리하므로 나는 **쓰기만** 한다(필드가 부족하면 인터페이스 변경 이슈로 요청)
...
### 🚫 하지 않는 것
파지·이송·적재 동작(F1), 닦기 힘제어(F3), HMI 화면(F4). 다른 노드 코드를 대신 만들지 않는다 — mock으로 대체.

### 📦 산출물 (상대경로)
`src/f2_sense_flow/f2_node.py` `flow_node.py` `mock/mock_f1_f3.py` `logger.py` · `config/params.yaml`의 `f2`·`flow` 절 · `records.csv` 스키마

### 🧪 단독 시험 리그 (남의 코드 없이)
100/200 g 추, 대용품 용기 4·빈 용기 4, 잔반통, 빈 수조. F1 없이 손으로 용기를 쥐여주고 서비스만 호출. flow는 mock으로.
...
9/18 B~C CELL-03 수조·대용품(V-11), C INF-03 mock·V-02, C~9/19 C FLOW-01, 9/19 A **V-20**·V-21(flow 실행 구조) → 9/19 B V-07, B~C F2-01 → 9/19 C~9/20 A FLOW-02 기록, A F2-02·UT-FLOW → 9/20 C **UT-F2**·**INT-12a 착수** → 9/21 C INT-12a 마무리·CR-01 → 9/22 B~C **L3 주도** → 9/23 **L4 주도**·동결
