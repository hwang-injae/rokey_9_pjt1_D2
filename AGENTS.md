...
2. **접촉 동작(탐색 하강·닦기·안착·삽입)에는 힘 상한 + 후퇴 + 타임아웃**을 항상 넣는다. 없으면 코드를 주지 않는다. 🔄 9/24 황인재(E44 · E50): 이 규칙은 **접촉 동작**(닦기 · 안착 탐색 · 바닥 찾기 · 힘으로 닿는 것)에 대한 것이다. **지정 좌표에 있는 물체로 가는 하강**(놓기 · 팔레트 적재 · 툴 반납 · 재파지)은 높은 접근점에서 자세를 맞춘 뒤 **Z 만 곧게** 내린다(마지막 15 mm 완충) — 물체는 항상 지정 좌표에 있고, 동선이 실기로 검증됐으면 조금씩 내려오며 힘을 볼 이유가 없다. 9/23 07:55 사고는 위치 편차가 아니라 관절 이동 동선 문제였고 이 규칙으로 고쳤다.
