# -*- coding: utf-8 -*-
"""시스템 아키텍처(PC 단위) — 노드=사각형, 토픽=타원, 화살표 라벨=서비스·메시지 타입.  SVG + draw.io 동시 생성.
...
# ---------- 노드(사각형) ----------
N('flow',430,124,260,70,['flow_node','f2_sense_flow · 민범진','유일한 호출자 · 상태 머신'],'f2')
N('f1',70,360,280,84,['f1_node','f1_handling · 한석형','탐색 파지 · 안착 놓기 · 이송 · 툴 · 팔레트 적재'],'f1')
N('f2',420,360,280,84,['f2_node','f2_sense_flow · 민범진','무게 · 잔반 폐루프 · 털기 · 헹굼'],'f2')
N('f3',770,360,280,84,['f3_node','f3_wipe · 박진용','세제 담금 · 그릇 닦기 · 컵 닦기'],'f3')
...
# ---------- 범례 ----------
N('legend',1140,300,280,214,['범례','','□ 사각형 = 노드','◯ 타원 = 토픽','→ 실선 화살표 = 서비스 호출(요청 방향)','     라벨 = 서비스 이름 : 타입','⇢ 점선 화살표 = 토픽 publish / subscribe','     라벨 = 메시지 타입','┄ 점선 상자 = 라이브러리(노드 아님)','━ 굵은 선 = 네트워크·물리 연결','색: 파랑 F1 · 초록 F2 · 주황 F3 · 보라 F4 · 회색 외부'],'lib','lib')

# ---------- 엣지 ----------
E('flow','f1',[(480,194),(480,222),(210,222),(210,360)],
  ['서비스 호출  flow_node → f1_node','/f1/pick : cobot_msgs/srv/F1Pick','/f1/place : cobot_msgs/srv/F1Place','/f1/move_to : cobot_msgs/srv/F1MoveTo','/f1/tool : cobot_msgs/srv/F1Tool','/f1/rack_place : cobot_msgs/srv/F1RackPlace'],(210,291))
...
E('flow','t_event',[(690,178),(1112,178),(1112,225),(1140,225)],['publish · cobot_msgs/msg/FlowEvent · 용기당 1건'],(900,165),'topic')
...
TITLE='PreWash-Cell 시스템 아키텍처 (PC 단위) — 노드 □ · 토픽 ◯ · 화살표 라벨 = 서비스/메시지 타입'
SUB='9/18 DSN-01 회의 반영: PC 2대 · 노드 5개 · flow_node만 호출 · wipe 분리 · seat/home 삭제 · 설정은 config/ 한 폴더.   미확정(DSN-03): 반납 구역 방식 · HMI 설계 · FlowState 필드'
NOTE='개발 중에는 mock_f1_f3(민범진)이 /f1/* · /f3/* 서비스를, fake_state_pub(황인재)이 /flow/state · /flow/event 를 같은 이름으로 대신 제공한다 → 로봇·남의 코드 없이 개발.   PC-A ↔ PC-B 는 같은 스위치의 DDS(멀티캐스트).'
