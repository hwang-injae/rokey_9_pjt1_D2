...
F1755 = '🚨 9/22 17:55 F4(V-25 컵 실기 16:45)'
P1755 = {
 'V-25':    dict(prog='0.6', status='진행', note_add=F1755 + ': 그릇 pick(#74) 첫 실기 OK(16:24 · 폭 2.32 · 홈 B 놓기 OK — 안전 정지 2회는 손으로 잘못 물린 그릇 탓, 로봇 집기로 바꾼 뒤 재발 없음). **컵 1차: grip 목표 76 · 5 N → 실제 107.8 mm(거의 열림) 인데 PickResult ok=True** — E19(컵 폭 판정 없음)라 성공으로 넘어감 · 컵은 안 잡힘. 2차(25 s 뒤) 76.2 정상. 기록은 V-24 결과까지 모아 문서 PR'),
...
for _tid, _e in P1755.items():
    EDIT.setdefault(_tid, {}).update(_e)
HISTORY124 = ['v17.4', '🚨 발견', 'V-25, F1-02, INF-02d, INT-12b', 'F4 V-25 컵 실기(9/22 16:45): 컵 pick 이 실제 폭 107.8(거의 열림)인데 ok=True — E19 로 폭 판정이 없어 안 잡힌 채 성공 처리. 가설 = gripper.py 힘 계단(_set_force)이 기억값과 어긋나 손가락이 안 움직임. 제안: 컵도 "안 닫힘" 최소 판정(목표 +10 mm 초과 → GRIP_FAIL) · 힘 set 확인 — 민범진 판단(18:20 묶음 맨 앞). 그릇 pick 첫 실기는 OK', '황인재 9/22 17:55', 'H,M']
...
P1735 = {
 'V-25':    dict(note_add=F1735 + ': 컵 1차 107.8 mm 는 **컵이 집는 자리에 딱 붙어 배치돼 손가락이 못 닫힌 것**(황인재 확인) — 힘 계단 가설 철회. "안 닫혀도 ok=True" 맹점은 그대로 → 민범진 판단 요청 유지. 🆕 **홈 C 놓기(place SPONGE_BED_C) 좌표가 안 맞는다**(16:46 관찰 · PlaceResult 는 OK 인데 컵이 홈에 제대로 안 앉음) — 방향·양 재실행 확인 중, 재현되면 F4 가 cell.yaml 고쳐 PR(E14)'),
 'INF-02d': dict(note_add=F1735 + ': 힘 계단(_set_force) 가설은 **철회**(컵 배치 문제였음) — ② 제안은 참고만'),
 'CELL-04': dict(note_add=F1735 + ': 🟡 `beds.SPONGE_BED_C.place`(#69 한석형: 접근 z 300 → 삽입 z 110 → +140) 로 놓으면 컵이 홈에 제대로 안 앉는다(황인재 16:46) — F4 재실행 확인 → 재현되면 좌표 수정 PR'),
...
# ---------------------------------------------------------------- 9/22 17:44 F4 — 🚨 툴·TCP 선택 두 번째 풀림(16:51~16:52 · 프로그램 안 돌던 틈) → 외부 요인 의심
T1744 = '🚨 9/22 17:44 F4'
P1744 = {
 'ENV-05':  dict(note_add=T1744 + ': **툴·TCP 선택이 오늘 두 번째로 풀렸다**(16:5x 확인 · get_current_tool/tcp 빈 값). 창 16:51:10(홈 C 놓기 posx 정상 완료 = TCP 살아 있음) ~ 16:52:27(다음 pick 부터 전혀 다른 좌표 · 16:53:59 move_stop 응답 없음). 그 사이 우리 프로그램은 안 돌았다 — 아침(11:10~11:17)도 rig_v03 종료 뒤 빈 틈 → **외부 요인(펜던트 조작 · 다른 PC 브링업/접속 · 컨트롤러) 의심**. 조치: 황인재가 펜던트에서 다시 선택(브링업 끄고 → 선택 → Dart 닫고 → 브링업 → 이름 확인) · 복구 전 로봇 이동 금지 · rig_f1 에 문지기 없어 못 막음 → F4 가 넣는다. 팀 질문: 16:51~16:53 펜던트·다른 PC 브링업/접속 여부(전원 · 즉시)'),
 'INT-F2':  dict(note_add=T1744 + ': 🚨 시작 전 **툴·TCP 두 이름 확인 필수**(16:5x 두 번째 풀림 · 복구 뒤) · 저녁 통합 준비로 다른 PC 에서 브링업·접속했는지 확인'),
 'INT-F3':  dict(note_add=T1744 + ': 🚨 시작 전 툴·TCP 두 이름 확인 필수 · rig_v03·rig_v10 은 아직 set/모드 전환 코드가 남아 있음(E26 수정 PR 전) — 실행 전 이름부터'),
 'V-25':    dict(note_add=T1744 + ': 상태 — 이동 3 · 빈손 놓기 · 그릇 pick→place 1(+손 1) · 컵 pick→홈 C 놓기 2회(좌표 어긋남 관찰 — 🟡 TCP 풀림 시각과의 관계 확인 필요: 16:46 관찰은 풀림(16:51) 전) · V-24 미실행 → 로봇을 통합 팀에 넘기면 V-24 는 9/23 아침'),
 'CELL-04': dict(note_add=T1744 + ': 홈 C 놓기 좌표 의심(16:46)은 TCP 풀림(16:51~) **전** 관찰이라 별개일 가능성 — F4 재실행으로 가린다'),
}
for _tid, _e in P1744.items():
    EDIT.setdefault(_tid, {}).update(_e)
HISTORY126 = ['v17.6', '🚨 사고 2', 'ENV-05, INT-F2, INT-F3, V-25, CELL-04', 'F4 9/22 17:44: 툴·TCP 선택이 두 번째로 풀림(16:51~16:52 · 우리 프로그램 안 돌던 틈 · 아침과 같은 양상) → 외부 요인 의심 · 황인재 펜던트 재선택 · 복구 전 이동 금지 · 저녁 통합 시작 전 두 이름 확인 필수 · 전원에게 "16:51~16:53 펜던트·다른 PC 브링업/접속 여부" 즉시 질문 · rig_f1 문지기 추가(F4)', '황인재 9/22 17:44', '전원']
...
EDIT.setdefault('ENV-05', {}).update(dict(note_add='🔎 17:55 실마리: 민범진 세션이 16:5x~17:07 실기 사용(dip 3회) → 16:51 TCP 풀림과 시각이 겹침 — 민범진 PC 의 sodreal 브링업 시각을 확인(다른 PC 연결이 이름을 비우는지)'))
...
EDIT.setdefault('ENV-05', {}).update(dict(note_add='🎯 17:58 F4 분석(원인 거의 확정): 황인재 V-25 16:24~16:55 실기 중에 민범진이 16:5x 다른 PC 에서 sodreal 접속 → ① 16:51~52 툴·TCP 빔 ② 16:52 황인재 명령에 로봇이 엉뚱한 좌표로(다른 PC 명령 실행) ③ 16:53:59 move_stop 무응답. 아침 11:1x = 박진용 rig_v03 다른 PC 접속과 겹침. 가설 "다른 PC 에서 sodreal 을 켜면 현재 툴·TCP 선택이 빈다" → 민범진 sodreal 시각으로 확정 대기. 🟡 규칙 제안 3개(선언 뒤 사용·접속 PC 1대 / 새 브링업은 두 이름 확인 / 펜던트 재선택 황인재) → 황인재 결정'))
EDIT.setdefault('V-25', {}).update(dict(note_add='⚠ 17:58: 16:46~16:51 컵 홈 C 놓기 어긋남 관찰은 민범진 접속과 겹친 시각일 수 있음 → cell.yaml 수정 보류 · 재확인은 9/23 아침 V-24 와 함께(로봇 확실히 빈 때)'))
EDIT.setdefault('CELL-04', {}).update(dict(note_add='17:58: F4 소유 시험대(rig_weigh_poses 등)에 go_home_safely 보호는 9/23 F4 가 넣음'))
HISTORY129 = ['v17.9', '원인', 'ENV-05', 'F4 9/22 17:58: 툴·TCP 풀림 2건 원인 = 다른 PC 의 sodreal 동시 접속(16:5x 민범진 · 11:1x 박진용)이 유력 — 접속이 넘어가며 선택이 비고 남의 명령이 실행됨 · 규칙 3개 제안(황인재 결정) · V-25 컵 홈 C 관찰 보류', '황인재 9/22 17:58', 'H']
