...
A('전원','계약','cobot_common v0 — move_to/move_rel/grip/release/weigh + config.py(로더) (F1 부담 분산으로 박진용이 작성, 좌표는 한석형의 cell.yaml)','P','INF-02','0C-1A','r','src/cobot_common/robot.py, config.py','Virtual에서 move_to·grip 동작','남들이 이걸로 개발')
...
A('전원','검증','V-20 여러 노드의 두산 API 동시 사용 — f1·f2·f3 세 프로세스가 같은 드라이버에 순차 명령(Virtual)','S,M','V-20','0C','g','검증 메모','세 노드에서 번갈아 movej 성공','안 되면 로봇 명령을 한 노드로 모음')
...
A('F2·flow','검증','V-21 서비스 콜백 안 장시간 모션 — rclpy 실행기 교착 여부(콜백 그룹·MultiThreadedExecutor)','M','V-21','1A','g','검증 메모','flow가 응답 받고 /flow/state 2 Hz 유지','mock · 안 되면 모션 스레드 분리')
