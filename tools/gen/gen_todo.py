...
'INF-02':'공용 로봇 함수 묶음(cobot_common)을 만든다. 맨 먼저 시작 함수 init(두산 API 준비 + 통신 노드 + 그리퍼 폭 받기)과 끝내기 shutdown, 그다음 이동·그리퍼 잡기/놓기·무게 재기 + 설정 파일 읽는 함수. 다른 사람들이 이걸 가져다 쓴다',
'TS-01':'로봇 프로그램이 뜨자마자 죽거나 두 번째 호출에서 멈추는 문제(TS-01)를 기록했다. 결론은 구조 변경(DSN-02b)',
'V-24':'로봇이 움직이는 도중에도 정지 버튼이 먹게 한다: 이동 명령을 보낸 뒤 짧게 반복 확인하면서 정지 깃발·시간 초과를 본다(가상 로봇)',
...
'INF-03':'로봇 없이 개발하려고 가짜를 만든다: F1·F3와 같은 이름의 가짜 함수 모듈(민범진), 가짜 상태 발행기(황인재)','INF-04':'설정 파일 2개(cell.yaml, params.yaml)의 뼈대와 한 번에 실행하는 런치 파일 2개를 만든다. 런치는 flow_node 프로그램 하나만 띄운다',
...
'V-23':'쥐고 있는 상태에서 힘만 올렸다 내렸다(보통↔강한 파지) 할 수 있는지, 어떤 방법으로 하는지 확인한다','V-20':'새 실행 구조가 팀 코드로도 도는지 가상 로봇에서 확인한다: flow_node가 세 사람의 함수를 번갈아 부르고, 그동안 상태 발행(초당 2번)과 정지 버튼이 살아 있는지, Ctrl+C로 끈 뒤 다시 켜지는지',
