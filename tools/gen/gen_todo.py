...
'V-24':'로봇이 움직이는 도중에도 일시 정지·재개·중단이 먹게 한다(코드는 끝남 — 비동기 이동 + 반복 확인, PR #34). 남은 것: 실기 확인 — 짧은 이동의 도착 위치, 이동 도중 일시 정지 → 재개, 접촉 동작 중 일시 정지',
...
'CELL-01':'작업대에 자리를 잡는다: 그릇·컵 반납 구역, 식기세척기용 팔레트(그릇 2칸·컵 2칸), 격리 구역. 그리퍼 손가락에 미끄럼방지 패드를 붙인다',
...
'V-17':'컵을 옆면으로 잡는 방식 확인(완료: 집기·이송은 OK, 털 때는 더 세게 잡아야 함)','V-19':'로봇 팔이 모든 작업 위치에 무리 없이 닿는지 확인한다(가상 로봇에서는 전부 닿음). 실기는 V-22 와 한 번에 — 흐름 순서대로 자리에서 자리로 곧장 가 보며 막히는 구간에 접근점을 추가한다','V-22':'설정 파일에 적은 좌표대로 로봇을 보냈을 때 실제 위치와 2 mm 이내로 맞는지 확인한다',
...
'V-10':'컵 안에 솔을 넣을 때 깊이와 부딪힘을 확인하고, 닦기 값 4개를 정한다: 위아래 진폭·좌우 비틀기 각·주기·바닥에서 띄우는 양','V-18':'닦는 동안(바닥에 닿은 채 문지르는 동안) 수세미 툴·솔이 그리퍼 안에서 밀리거나 돌지 않는지 본다 — 닦기 3회를 돌릴 때 같이','V-13':'브라우저에서 시작 버튼을 누르면 가짜 flow가 반응하는지 확인한다','V-14':'그릇 2개를 겹치거나 어긋나게 놓고 "찾아서 집기"를 10번 해 성공률을 잰다',
...
'F4-00':'HMI를 어떻게 만들지 1장으로 설계한다: 화면 구성, 서버 주소(REST/WS), 화면에 필요한 상태 값','F4-01':'가짜 상태 발행기(fake_state_pub)와 FastAPI 첫 페이지를 만든다','F4-02':'버튼 4개(시작·일시 정지·재개·중단)를 누르면 flow 의 서비스가 불리고, 상태가 실시간(WebSocket)으로 화면에 온다(완료 — PR #41). 웹 화면은 이 PC 에서만 열린다',
