...
'INF-02':'공용 로봇 함수 묶음(cobot_common)을 만든다: 이동·그리퍼 잡기/놓기·무게 재기 + 설정 파일 읽는 함수. 다른 사람들이 이걸 가져다 쓴다',
...
'V-23':'쥐고 있는 상태에서 힘만 올렸다 내렸다(보통↔강한 파지) 할 수 있는지, 어떤 방법으로 하는지 확인한다','V-20':'f1·f2·f3 세 프로그램이 같은 로봇 드라이버에 번갈아 명령을 보내도 되는지 가상 로봇으로 확인한다. 안 되면 구조를 바꿔야 한다',
'V-21':'몇 초~수십 초 걸리는 로봇 동작을 서비스 안에서 실행할 때 프로그램이 멈추지 않는지(가짜 노드로) 확인한다','V-02':'100 g·200 g 추를 들고 로봇이 재는 무게가 ±20 g 안에 들어오는지 10번 잰다','V-07':'털기 동작을 했을 때 로봇이 "충돌"로 오해해 멈추지 않는지 확인한다',
...
def where(tid):
    t=tid or ''
    rules=[(r'^V-','docs/03_설계_SDD.md §9.2 (통과 기준·안 되면 대안)'),(r'^UT-','SDD §9.3 (TC 표)'),(r'^INT-1|^INT-4$','SDD §9.4 (L2 통합)'),(r'^INT-3','SDD §9.5'),(r'^INT-4[a-d]','SDD §9.6~9.7'),
      (r'^F1-','SDD §5.2 · IRD §3'),(r'^F2-','SDD §5.3 · IRD §4'),(r'^FLOW','SDD §5.1 · IRD §8'),(r'^F3-','SDD §5.4 · IRD §5'),(r'^F4-','SDD §5.5·§6 · IRD §6~7'),
      (r'^INF-02','SDD §3.1 (함수 표)'),(r'^INF-04','SDD §4.3 · IRD §9'),(r'^INF-01','IRD · docs/interfaces/'),(r'^INF-03','IRD §10'),(r'^CELL','SDD §2 (워크셀)'),(r'^DSN','docs/meetings/'),
