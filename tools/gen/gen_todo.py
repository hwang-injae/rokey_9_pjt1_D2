...
PEOPLE=[('S','한석형','팀장 · F1 파지·이송·적재 + 좌표 계산·티칭'),('M','민범진','F2 무게·털기·헹굼 + flow_node + mock · 통합 실행 리더'),
        ('P','박진용','F3 접촉 닦기 + cobot_common(공용 로봇 함수) + 안전 파라미터'),('H','황인재','PM · F4 웹 HMI + cobot_msgs·런치·일정표·제출')]
...
'INF-01':'서비스·메시지 정의를 ROS 패키지(cobot_msgs)로 만들어 배포한다','INF-01b':'회의 결과대로 cobot_msgs를 고친다(닦기 서비스 둘로 분리, seat 삭제, place 응답에 offset 추가)',
'INF-02':'공용 로봇 함수 묶음(cobot_common)을 만든다. 맨 먼저 노드 시작 함수 init·spin(이게 없으면 로봇 노드가 죽거나 멈춘다, TS-01), 그다음 이동·그리퍼 잡기/놓기·무게 재기 + 설정 파일 읽는 함수. 다른 사람들이 이걸 가져다 쓴다',
'TS-01':'로봇 노드가 뜨자마자 죽거나 두 번째 호출에서 멈추는 문제(TS-01)를 기록하고, 해결 방법(노드 뼈대 규약)을 문서·프롬프트에 반영해 팀에 알린다',
'INF-02b':'cobot_common에 힘 관련 함수를 넣는다: 힘제어 켜기/끄기, 닿을 때까지 내려가기, 흔들며 찾기',
'INF-03':'로봇 없이 개발하려고 가짜 노드를 만든다: 가짜 F1·F3 서비스(민범진), 가짜 상태 발행기(황인재)','INF-04':'설정 파일 2개(cell.yaml, params.yaml)의 뼈대와 한 번에 실행하는 런치 파일 2개를 만든다',
'CELL-04':'다 같이 로봇을 손으로 움직여 각 위치(홈·무게 재는 곳·잔반통·스펀지 홈·툴 홀더·수조·팔레트)의 좌표를 읽어 cell.yaml에 적는다',
'ARCH-01':'1차 회의 결과를 draw.io 아키텍처 그림으로 정리하고, 강사가 요구한 산출물 10종을 노션에 올린다',
'DSN-03':'2차 회의: 반납 구역 방식, HMI 설계, 실패했을 때 처리 규칙, 설정 키 이름 규칙을 정한다','DSN-04':'2차 회의에서 정한 것을 문서와 cobot_msgs에 반영한다',
'MID-01':'월요일 중간점검 발표 자료 10장을 만든다(주제·아키텍처·역할·진행·계획)','MID-02':'중간점검: 30분 발표 + 30분 토론. 강사 피드백을 받아 적는다','CR-01':'서로의 코드를 본다: 안전(힘 상한·후퇴·타임아웃), 숫자 하드코딩, 인터페이스',
'NOTE-01':'노션에 ROS 2 노드 구조와 인터페이스 정의서를 올린다(강사 9/22 요구)','NOTE-02':'노션에 HMI 화면을 gif로 올린다(강사 9/22 요구)',
'INT-3a':'그릇 1개를 처음부터 끝까지(집기→무게→닦기→헹굼→적재) 실제 노드와 HMI로 3번 연속 성공시킨다','INT-3b':'컵 1개를 처음부터 끝까지 3번 연속 성공시킨다','FIX-01':'통합에서 나온 문제를 고치고 값(힘·속도·좌표)을 다듬는다',
...
'V-01':'그리퍼로 그릇·컵을 잡았을 때와 빈손일 때의 "벌어진 폭" 값이 확실히 다른지 잰다. 이게 돼야 "잡았는지"를 알 수 있다','V-05':'로봇의 신호선(DO1/DO2)으로 그리퍼가 잡기/놓기를 하고, 완료 신호(DI1/DI2)가 돌아오는지 확인한다',
...
'V-23':'쥐고 있는 상태에서 힘만 올렸다 내렸다(보통↔강한 파지) 할 수 있는지, 어떤 방법으로 하는지 확인한다','V-20':'f1·f2·f3 세 프로그램을 정해진 뼈대(SDD §3.2)로 만들어, 가상 로봇에 번갈아 명령을 보낸다. 프로그램마다 같은 서비스를 3번 연속 불러 본다(한 번만 부르면 결함이 안 보인다)',
'V-21':'flow_node가 다른 노드를 부르고 기다리는 동안에도 상태 발행(초당 2번)과 정지 버튼이 살아 있는지 가짜 노드로 확인한다. 순서 실행은 별도 스레드에서 한다(SDD §5.1)','V-02':'100 g·200 g 추를 들고 로봇이 재는 무게가 ±20 g 안에 들어오는지 10번 잰다','V-07':'털기 동작을 했을 때 로봇이 "충돌"로 오해해 멈추지 않는지 확인한다',
...
'UT-F1':'F1 단위 테스트(TC-01 찾아서 집기, 02 툴, 05 안착 놓기, 09 팔레트)를 기준대로 통과시킨다','INT-12b':'F1+F2 통합: 다시 집기→헹굼→물 털기→팔레트 적재를 5번 연속','INT-13p':'F1+F3 통합에 F1 담당으로 참여(안착 놓기·툴)',
'CELL-03':'잔반통과 수조 2개를 로봇 컨트롤러 반대편에 고정하고, 잔반 대용품(구슬·쌀 등 100 g 이상)을 정한다','FLOW-01':'전체 순서를 지휘하는 flow_node를 가짜 노드로 먼저 만든다: 단계 전환, 실패 시 재시도·격리·정지, 시작/정지/재개',
...
'UT-FLOW':'flow 단위 테스트(TC-10 실패 규칙, TC-12 기록)를 가짜 노드로 통과시킨다','UT-F2':'F2 단위 테스트(TC-03 무게, 04 잔반, 08 헹굼)를 통과시킨다','INT-12a':'F1+F2 통합: 찾아서 집기→무게→털기를 5번 연속',
'SAFE-01':'안전 값(속도·힘 상한·타임아웃) 표와 위험요소·대책, 오류 목록을 정리해 노션에 올린다','CELL-02a':'그릇·컵 치수를 재고 스펀지 홈을 어떻게 자를지 그림을 그린다. 툴 홀더 위치도 정한다','CELL-02':'스펀지에 그릇·컵 모양 홈을 파고, 툴 홀더 2개와 수세미 손잡이를 만들어 작업대에 고정한다',
'F3-02':'그릇 닦기(wipe_bowl)를 만든다: 일정한 힘으로 누르며 나선으로 닦고 힘 기록을 남긴다. 힘이 넘거나 오래 걸리면 즉시 뒤로','F3-03':'세제 담금(soap)과 컵 닦기(wipe_cup: 솔을 넣어 돌리고 위아래)를 만든다','UT-F3':'F3 단위 테스트(TC-06 그릇 닦기, 07 컵 닦기)를 통과시킨다','INT-13':'F1+F3 통합: 안착 놓기→툴 집기→세제→닦기→툴 반납을 5번 연속',
'F4-00':'HMI를 어떻게 만들지 1장으로 설계한다: 화면 구성, 서버 주소(REST/WS), 화면에 필요한 상태 값','F4-01':'가짜 상태 발행기와 FastAPI 첫 페이지를 만든다','F4-02':'버튼 3개(시작·정지·재개)와 실시간 상태 전달(WebSocket)을 만든다. 정지 버튼은 항상 보이게',
...
def where(tid):
    t=tid or ''
    rules=[(r'^V-20','SDD §3.2 (노드 뼈대) · §9.2'),(r'^V-','docs/03_설계_SDD.md §9.2 (통과 기준·안 되면 대안)'),(r'^UT-','SDD §9.3 (TC 표)'),(r'^INT-1|^INT-4$','SDD §9.4 (L2 통합)'),(r'^INT-3','SDD §9.5'),(r'^INT-4[a-d]','SDD §9.6~9.7'),
      (r'^F1-','SDD §5.2 · IRD §3'),(r'^F2-','SDD §5.3 · IRD §4'),(r'^FLOW','SDD §5.1 · IRD §8'),(r'^F3-','SDD §5.4 · IRD §5'),(r'^F4-','SDD §5.5·§6 · IRD §6~7'),
      (r'^INF-02','SDD §3.1 (함수 표)'),(r'^INF-04','SDD §4.3 · IRD §9'),(r'^INF-01','IRD · docs/interfaces/'),(r'^INF-03','IRD §10'),(r'^CELL','SDD §2 (워크셀)'),(r'^DSN','docs/meetings/'),(r'^TS-','docs/troubleshooting/'),(r'^V-20','SDD §3.2 (노드 뼈대) · §9.2'),
      (r'^ENV','docs/setup/M0609_환경설정.md'),(r'^SAFE','SDD §8'),(r'^ARCH','SDD §1 · docs/images/*.drawio'),(r'^NOTE','SDD §12 (강사 산출물 표)'),(r'^(DOC|REH|SUB|MID|DEMO|WRAP)','일정표 "마일스톤·로봇 슬롯" 시트'),(r'^(BRF|PM)','일정표 "규칙" 시트'),(r'^CR-','CONTRIBUTING.md §4·§8'),(r'^FIX','SDD §9.9 (범위 방어)')]
    for pat,w in rules:
