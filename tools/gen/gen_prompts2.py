...
- **검증(V)**: 9/18 저녁 V-01 파지 폭 3상태(그릇·컵·빈손)·V-05 DO/DI·V-17 컵 옆면 파지(✅ 9/18 검증 완료 — 흔드는 동작용 강한 파지는 V-16·V-23)·V-20 여러 노드의 두산 API 동시 사용(Virtual, 민범진과). 9/19 오전 **공통 좌표 티칭 세션** + V-19 도달 범위·특이점 + V-22 티칭 좌표 재현 오차. 9/19 오후 V-06 삽입·V-08 툴·**V-15 재파지 폭 인식(박진용과)**·V-16 파지력·미끄러짐(민범진과). 9/20 오전 **V-14 겹친 용기 탐색 파지**. 반납 구역 방식은 9/19 저녁 DSN-03에서 확정되므로 `pick` 탐색 구현(F1-02)은 그 뒤에 굳힌다
...
- **검증(V)**: 9/18 저녁 V-02 하중 정밀도·V-20 여러 노드의 두산 API 동시 사용(Virtual, 한석형과). 9/19 오전 V-21 서비스 콜백 안 장시간 모션(실행기 교착 여부, mock). 9/19 오후 V-07 털기 충돌 감지·V-16 파지력·미끄러짐(한석형과)
...
- 겸임: 통합 리더 — L2 INT-12a 주도, L3·L4 **실행** 주도(통합 체크리스트·범위 방어 발동 제안). 일정표·문서·`cobot_msgs`·런치·노션 업로드·제출은 PM(황인재) 담당이므로 진척 보고만 전달''',
 ...
 sched='9/18 B~C CELL-03 수조·대용품(V-11), C INF-03 mock·V-02·V-20, C~9/19 C FLOW-01, 9/19 A V-21 → 9/19 B V-07, B~C F2-01 → 9/19 C~9/20 A FLOW-02 기록, A F2-02·UT-FLOW → 9/20 C **UT-F2**·**INT-12a 착수** → 9/21 C INT-12a 마무리·CR-01 → 9/22 B~C **L3 주도** → 9/23 **L4 주도**·동결',
...
- **9/18 저녁(재료 없이 할 수 있는 것)**: ① **V-03 힘제어 중 XY 이동 가능 여부**(로봇 슬롯: 민범진 V-02 다음. 아무 스펀지 + 그릇으로. 결과가 닦기 설계를 정한다) ② **`cobot_common` 전체 작성(INF-02 + INF-02b, 최우선 — 남들이 이걸로 개발)**: `move_to(station, carrying)` `move_rel` `grip(width, force)→width` **`grip_level(kind, 'NORMAL'|'HOLD')`**(털기·헹굼용 강한 파지 전환, 방법은 V-23: 강사 배포 `onrobot_rg_control`이 힘 0~40 N을 명령마다 받음) `release()` `weigh(n)` `force_on/off` `contact_down(max_depth, limit)` `periodic_search` `safe_retreat` + `config.py`(config/ 폴더 로더). 좌표 값은 한석형의 `cell.yaml`을 읽기만 한다. 두산 API를 직접 부르는 곳은 이 패키지뿐 ③ SAFE-01 안전 파라미터 표·위험요소/안전대책·예외 리스트 ④ 그릇·컵 치수 측정과 스펀지 홈 커팅 도면·홀더 위치 결정
...
- 겸임: 안전 파라미터(속도·충돌 감도·힘 상한·수조 배치) 소유, SAFE-01 위험요소·안전대책·예외/오류 리스트(9/18 저녁 작성 → 노션 업로드)''',
 ...
 sched='9/18 C **V-03**·**INF-02/02b cobot_common**(9/19 오전까지)·SAFE-01·치수 측정·커팅 도면 → 9/19 A CELL-02 고정틀·홀더(재료 도착)·V-12, B **V-04·V-10·V-15·V-18**, C F3-02 wipe_bowl 착수 → 9/20 A~B F3-02 wipe_bowl, B F3-03 soap·wipe_cup·**UT-F3** → 9/21 C~9/22 A **INT-13 주도** → 9/22 L3 → 9/23 L4 실패 주입',
